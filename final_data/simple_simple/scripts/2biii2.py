...
os.system('mkdir -p ../plots/2biii2')
...
for csv in csvs:
    ...
    for col in range(1, table.shape[1]):
        plt.scatter(table.iloc[:, 0], table.iloc[:, col], label=table.columns[col])
    ...
    plt.legend()
    plt.legend(facecolor = color_values[random.randint(0, len(color_values) - 1)])

    plt.ylabel(y_label)
    plt.title(textwrap.fill(title,120))
    plt.tight_layout()
    plt.savefig(os.path.join('../plots/2biii2', csv.replace('.csv', '.png')))
    plt.close()