...
for csv in csvs:
    ...
    for col in range(1, table.shape[1]):
        plt.plot(table.iloc[:, 0], table.iloc[:, col], label=table.columns[col], linestyle=linestyle[1], color=color, marker=marker_list[col - 1])
    ...
    plt.tight_layout()
    plt.savefig(os.path.join('../plots/4a1', csv.replace('.csv', '.png')))
    plt.close()