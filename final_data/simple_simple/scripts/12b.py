...
for csv in csvs:
    ...
    colors_sample = random.sample(colors, table.shape[1] - 1)
    ...
    for col in range(1, table.shape[1]):
        axes[col - 1].plot(table.iloc[:, 0], table.iloc[:, col], label=table.columns[col], color=colors_sample[col - 1])
    
    for i in range(3, table.shape[1]):
        col = i - 1
        if col % 2 == 0:
            axes[col].spines['right'].set_position(('axes', (1.05 + (0.5 * ((col - 2) // 2)))))
        else:
            axes[col].spines['right'].set_position(('axes', (-0.07 - (0.5 * ((col - 2) // 2)))))

    for col in range(1, table.shape[1]):
        axes[col-1].set_ylabel('Value', color=colors_sample[col - 1])
    ...
    plt.tight_layout()
    plt.savefig(os.path.join('../plots/12b', csv.replace('.csv', '.png')))
    plt.close()
