...
for csv in csvs:
    ...
    for col in range(1, table.shape[1]):
        plt.fill_between(table.iloc[:, 0], table.iloc[:, col], alpha=0.5, label=table.columns[col])
    ...
    plt.tight_layout()
    plt.savefig(os.path.join('../plots/4gi', csv.replace('.csv', '.png')))
    plt.close()