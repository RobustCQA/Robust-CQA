...
for csv in csvs:
    ...
    fig, ax = plt.subplots(num_plots, 1, figsize=(10 * num_plots, 5))

    for col in range(1, table.shape[1]):
        ax[col - 1].plot(table.iloc[:, 0], table.iloc[:, col], label=table.columns[col])
        ax[col - 1].set_title(textwrap.fill(table.columns[col],60))
        ax[col - 1].set_ylabel(y_label)
        if table.iloc[:, 0].dtype == 'O':
            ax[col - 1].set_xticklabels([textwrap.fill(e, 6) for e in table.iloc[:, 0]], rotation=0)
        else:
            ax[col - 1].set_xticklabels(table.iloc[:, 0], rotation=0)
        ax[col - 1].legend()

    fig.suptitle(textwrap.fill(title,120))
    plt.savefig(os.path.join('../plots/10aii', csv.replace('.csv', '.png')))
    plt.close()