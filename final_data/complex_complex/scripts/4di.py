...
for csv in csvs:
    ...
    marker_list = random.sample(markers, table.shape[1] - 1)
    color_list = random.sample(colors, table.shape[1] - 1)

    plt.figure(figsize=(20, 10))

    for col in range(1, table.shape[1]):
        plt.stem(table.iloc[:, 0], table.iloc[:, col], label=table.columns[col], linefmt=color_list[col - 1], markerfmt=marker_list[col - 1], basefmt='k-')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join('../plots/4di', csv.replace('.csv', '.png')))
    plt.close()