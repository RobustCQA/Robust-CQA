...
for csv in csvs:
    ...
    for col in range(1, table.shape[1]):
        plt.bar(table.iloc[:, 0], table.iloc[:, col], label=table.columns[col], alpha=0.5)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join('../plots/4bi', csv.replace('.csv', '.png')))
    plt.close()