...
for csv in csvs:
    ...
    for bar in range(1, table.shape[1]):
        ax.bar(x + width * (bar - 1), table.iloc[:, bar], width, label=table.columns[bar], hatch=hatch)
    ...
    fig.tight_layout()
    plt.savefig(os.path.join('../plots/3b', csv.replace('.csv', '.png')))
    plt.close()
