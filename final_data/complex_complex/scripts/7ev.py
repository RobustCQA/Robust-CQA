...
for csv in csvs:
    ...
    for col in range(1, table.shape[1]):
        plt.plot(table.iloc[:, 0], table.iloc[:, col], label=table.columns[col], color=color_values[color_starting + col - 1])
    ...
    plt.grid(which='both', axis='both', color=color_values[color_starting + table.shape[1] - 1], linestyle='-', linewidth=0.5)
    
    plt.ylabel(y_label)
    plt.title(textwrap.fill(title,120))
    plt.tight_layout()
    plt.savefig(os.path.join('../plots/7ev', csv.replace('.csv', '.png')))
    plt.close()