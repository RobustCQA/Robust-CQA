...
for csv in csvs:
    ...
    for col in range(1, table.shape[1]):
        plt.scatter(table.iloc[:, 0], table.iloc[:, col], label=table.columns[col], color=color, marker=marker, s=100 * col)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join('../plots/4cii', csv.replace('.csv', '.png')))
    plt.close()