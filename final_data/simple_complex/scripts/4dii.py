...
for csv in csvs:
    ...
    for col in range(1, table.shape[1]):
        plt.stem(table.iloc[:, 0], table.iloc[:, col], label=table.columns[col], linefmt=color_list[col - 1], markerfmt=marker, basefmt='k-')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join('../plots/4dii', csv.replace('.csv', '.png')))
    plt.close()