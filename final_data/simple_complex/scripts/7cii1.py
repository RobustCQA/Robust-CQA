...
for csv in csvs:
    ...
    # annotating the lines with the column names and avoiding overlap and using text wrap
    for i in range(1, table.shape[1]):
        plt.text(table.iloc[-1, 0], table.iloc[-1, i], table.columns[i], fontsize=14, ha='left', va='center')

    plt.ylabel(y_label)
    plt.title(textwrap.fill(title,120))
    plt.tight_layout()
    plt.savefig(os.path.join('../plots/7cii1', csv.replace('.csv', '.png')))
    plt.close()