...
for csv in csvs:
    ...
    for col in range(1, table.shape[1]):
        plt.bar(table.iloc[:, 0], table.iloc[:, col], label=table.columns[col], bottom=table.iloc[:, 1:col].sum(axis=1))
    ...
    plt.tight_layout()
    plt.savefig(os.path.join('../plots/4bii', csv.replace('.csv', '.png')))
    plt.close()