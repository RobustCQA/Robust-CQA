...
for csv in csvs:
    ...
    if table.iloc[:, 0].dtype == 'O':
        plt.xticks(table.iloc[:, 0], [textwrap.fill(e, 8) for e in table.iloc[:, 0]], rotation=45)
    else:
        plt.xticks(table.iloc[:, 0], rotation=45)

    plt.legend()
    plt.ylabel(y_label)
    plt.title(textwrap.fill(title,120))
    plt.tight_layout()
    plt.savefig(os.path.join('../plots/7di1', csv.replace('.csv', '.png')))
    plt.close()