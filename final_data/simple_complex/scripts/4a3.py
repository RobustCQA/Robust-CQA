...
for csv in csvs:
    ...
    if table.shape[1] < len(linestyle_tuple) + 1:
        linestyle_list = random.choices(linestyle_tuple, k=table.shape[1] - 1)
    else:
        cant_make.append(csv)
        continue

    plt.figure(figsize=(20, 10))

    for col in range(1, table.shape[1]):
        plt.plot(table.iloc[:, 0], table.iloc[:, col], label=table.columns[col], linestyle=linestyle_list[col - 1][1], color=color, marker=marker)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join('../plots/4a3', csv.replace('.csv', '.png')))
    plt.close()
