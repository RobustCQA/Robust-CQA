...
for csv in csvs:
    ...
    col = table.shape[1] - 1
    plt.stackplot(table.iloc[:, 0], table.iloc[:, 1:col + 1].T, labels=table.columns[1:col + 1])
    ...
    plt.tight_layout()
    plt.savefig(os.path.join('../plots/4fi', csv.replace('.csv', '.png')))
    plt.close()