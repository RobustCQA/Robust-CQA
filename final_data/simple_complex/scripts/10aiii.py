...
os.system('mkdir -p ../plots/10aiii')
...
for csv in csvs:
    ...
    for col in range(1, table.shape[1]):
        ax[(col - 1) // nearest_sq, (col - 1) % nearest_sq].plot(table.iloc[:, 0], table.iloc[:, col], label=table.columns[col])
        ...
        if table.iloc[:, 0].dtype == 'O':
            ax[(col - 1) // nearest_sq, (col - 1) % nearest_sq].set_xticklabels([textwrap.fill(e, 6) for e in table.iloc[:, 0]], rotation=0)
        else:
    ...
    plt.savefig(os.path.join('../plots/10aiii', csv.replace('.csv', '.png')))
    plt.close()