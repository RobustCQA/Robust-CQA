...
for csv in csvs:
    ...
    for col in range(1, table.shape[1]):
        ...
        if choice == 0:
            ...
            ax[(col - 1) // nearest_sq, (col - 1) % nearest_sq].bar(table.iloc[:, 0], table.iloc[:, col], label=table.columns[col])
    ...
    plt.savefig(os.path.join('../plots/10ax', csv.replace('.csv', '.png')))
    plt.close()