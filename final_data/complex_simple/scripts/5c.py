...
for csv in csvs:
    ...
    for col in range(0, newtable.shape[1]):
        plt.scatter(newtable.index, newtable.iloc[:, col], label=newtable.columns[col], color = colors[col % len(colors)])
    ...
    plt.tight_layout()
    plt.savefig(os.path.join('../plots/5c', csv.replace('.csv', '.png')))
    plt.close()